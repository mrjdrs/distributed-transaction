...
public class StockService {
    ...
    public void updateStock(OrderEntity order, Channel channel, Message message) throws IOException {
        ...
        if (order.getId() == 0) {
            ...
            amqpTemplate.convertAndSend(ExchangeConstant.TX_RETURN_EXCHANGE, QueueConstant.TX_ORDER_QUEUE_RETURN_FAIL,
                    order.getId());
            channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
            throw new RuntimeException("============修改库存失败");
        } else {
            // 库存系统修改库存成功，则向MQ中发送成功订单ID
            amqpTemplate.convertAndSend(ExchangeConstant.TX_RETURN_EXCHANGE, QueueConstant.TX_ORDER_QUEUE_RETURN_SUCCESS,
                    order.getId());
//            channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
        }
    }
